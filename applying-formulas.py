...
class SienaIntro(Scene):
	...
	#	self.play(FadeOut(square))
		...
		words1 = Text("A race car starting from rest accelerates uniformly at a rate of\n4.9 meters per second squared. What is the car's speed after\nit has traveled 200 meters?").scale(.6)
		...
		self.wait()
		last = Text("This is the simplest equation to use because it only\ncontains given variables and the unknown.\nThe other two equations include time which we\ndo not know.").scale(.5).set_color(YELLOW)
		last.next_to(eq3,DOWN*2)
		self.play(Write(last,run_time=4))
		self.wait(5)
		newmethod = Text("While this may be the simplest method, there are\nother options. We could calculate time first and use\nit as a new known variable in a different equation.").set_color(BLUE)
		...
		newmethod.scale(.8)
		self.play(Write(newmethod))
		self.wait(6)
		self.remove(newmethod)
		words1[78:98].set_color(WHITE)
		self.add(prob1, words1,givens1, unknown, vi1, a1, d1)
		time = Tex("$t = ?$").next_to(unknown,DOWN).set_color(GREEN).scale(.8)
		...
		eq3[0].set_color(WHITE)
		self.add(eq1,eq2,eq3)
		framebox6 = SurroundingRectangle(time,buff=.1).set_color(GREEN)
		self.play(Create(framebox6))
		self.wait()
		eq1[5].set_color(GREEN)
		self.wait()
		eq2[3].set_color(GREEN)
		self.wait()
		eq2[7].set_color(GREEN)
		self.wait(2) 
		framebox7 = SurroundingRectangle(eq2,buff=.1)
		self.play(Create(framebox7))
		self.wait(2)
		eq2.scale(1.5)
		framebox7.scale(1.5)
		eq2.move_to(RIGHT*3)
		framebox7.move_to(RIGHT*3)
		self.wait(2)
		last2 = Text("We can now calculate time using this equation.\nWith time as a known value, any equation can\nbe used to calculate the final velocity. ").scale(.5).set_color(YELLOW)
		last2.next_to(eq2,DOWN*2)
		self.play(Write(last2,run_time=4))
		self.wait(7)
		

		self.remove(last2,prob1,words1,givens1,vi1,a1,d1,unknown,time,framebox6,framebox7,eq1,eq2,eq3,options)
